          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','store_name') || ($check_field('add','store_name') || $check_field('set','store_name'))" label="店铺名称" name="store_name">
                            <uni-easyinput type="text" v-model="form['store_name']" v-if="user_group === '管理员' || (form['food_information_id'] && $check_field('set','store_name')) || (!form['food_information_id'] && $check_field('add','store_name'))" :disabled="disabledObj['store_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','merchant_users') || ($check_field('add','merchant_users') || $check_field('set','merchant_users'))" label="商家用户" name="merchant_users">
                  ...
                  v-if="user_group === '管理员' || (form['food_information_id'] && $check_field('set','merchant_users')) || (!form['food_information_id'] && $check_field('add','merchant_users'))"
                ></uni-data-select>
                <uni-data-select
                  v-model="form['merchant_users']"
                  :localdata="list_user_merchant_users"
              ...
              <uni-forms-item v-if="$check_field('get','food_types') || ($check_field('add','food_types') || $check_field('set','food_types'))" label="美食类型" name="food_types">
                  ...
                  :localdata="list_food_types"
                  :clear="!disabledObj['food_types_isDisabled']"
                  :disabled="disabledObj['food_types_isDisabled']"
              ...
              <uni-forms-item v-if="$check_field('get','food_prices') || ($check_field('add','food_prices') || $check_field('set','food_prices'))" label="美食价格" name="food_prices">
                            <uni-easyinput type="text" v-model="form['food_prices']" v-if="user_group === '管理员' || (form['food_information_id'] && $check_field('set','food_prices')) || (!form['food_information_id'] && $check_field('add','food_prices'))" :disabled="disabledObj['food_prices_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','food_prices')">
                  {{ form['food_prices'] }}
                </text>
                          </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','food_pictures') || ($check_field('add','food_pictures') || $check_field('set','food_pictures'))" label="美食图片" name="food_pictures">
                    <!-- 修改权限 -->
                <view class="diy_field diy_img" v-if="form['food_pictures'] && $check_field('set','food_pictures')">
                  <image v-if="disabledObj['food_pictures_isDisabled']" :src="$fullUrl(form['food_pictures'])" />
                  <image v-if="!disabledObj['food_pictures_isDisabled']" :src="$fullUrl(form['food_pictures'])" @click="change_img('food_pictures')" />
                </view>
                <!-- 添加权限 -->
                <view class="diy_field diy_img" v-else-if="!form['food_pictures'] && $check_field('add','food_pictures')">
                  <view v-if="disabledObj['food_pictures_isDisabled']" class="btn_add_img">
                    <text>+</text>
                  </view>
                  <view v-if="!disabledObj['food_pictures_isDisabled']" class="btn_add_img" @click="change_img('food_pictures')">
                    <text>+</text>
                  </view>
                </view>
                <!-- 查询权限 -->
                <view class="diy_field diy_img" v-else-if="$check_field('get','food_pictures')">
                  <image :src="$fullUrl(form['food_pictures'])" />
                </view>
                  </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','food_introduction') || ($check_field('add','food_introduction') || $check_field('set','food_introduction'))" label="美食介绍" name="food_introduction">
                    <uni-easyinput type="textarea" v-model="form['food_introduction']" v-if="user_group === '管理员' || (form['food_information_id'] && $check_field('set','food_introduction')) || (!form['food_information_id'] && $check_field('add','food_introduction'))" :disabled="disabledObj['food_introduction_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','food_introduction')">
...
export default {
  ...
  methods: {
    ...
    async get_list_food_types() {
      ...
      if(json.result && json.result.list){
        json.result.list.map((o) => this.list_food_types.push({value:o.food_types,text:o.food_types}));
      }
      else if(json.error){
        console.error(json.error);
      }
