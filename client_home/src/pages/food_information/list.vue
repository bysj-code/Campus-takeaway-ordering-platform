			...
			<view class="container">
				<view>
					<view>
						<view class="">
							<!-- 搜索栏 -->
													<uni-search-bar placeholder="搜索店铺名称" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'store_name')">
				...
				<view>
					<view>
						<view class="tab_view">
							<!-- 筛选栏 -->
																						<uni-section title="美食类型" type="line">
							<uni-data-select
									@change="change_food_types"
									v-model="query['food_types']"
		...
		<!-- 列表 -->
			...
			<view class="container">
				<view v-for="(o,i) in list" class="row box_style">
							<view class="col" v-if="1 && $check_field('get','store_name')">
						<view class="view">
						...
						<view class="view">
							...
							<view class="col" v-if="1 && $check_field('get','food_types')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>美食类型</span>
							</view>
							...
							<view class="col" v-if="1 && $check_field('get','food_prices')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>美食价格</span>
							</view>
							...
							<view class="col" v-if="1 && $check_field('get','food_pictures')">
						<view class="view">
							<view class="diy_title" v-if=" false">
								<span>美食图片</span>
							</view>
										<view class="diy_field diy_img">
								<img :src="$fullUrl(o['food_pictures'])" width="100%" height="100" />
							</view>
									</view>
					</view>
							<view class="col" v-if="0 && $check_field('get','food_introduction')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>美食介绍</span>
							</view>
...
<script>
			...
			return {
				url_get_list: "~/api/food_information/get_list?like=0",
					...
					list_user_merchant_users: [],
										// 美食类型选项列表
				list_food_types: [{value:"全部",text:"全部"}],
	...
	get_user_merchant_users(id){
		...
		return ret;
						...
						// 改变分类标签
	change_food_types(val) {
		if (val === "全部" || val === "") {
			this.query.food_types = "";
		}else{
			this.query.food_types = val
		}
		this.search();
	},
