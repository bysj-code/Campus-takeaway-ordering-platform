          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','user_name') || ($check_field('add','user_name') || $check_field('set','user_name'))" label="用户姓名" name="user_name">
                            <uni-easyinput type="text" v-model="form['user_name']" v-if="user_group === '管理员' || (form['delivery_users_id'] && $check_field('set','user_name')) || (!form['delivery_users_id'] && $check_field('add','user_name'))" :disabled="disabledObj['user_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','user_gender') || ($check_field('add','user_gender') || $check_field('set','user_gender'))" label="用户性别" name="user_gender">
                  ...
                  v-if="user_group === '管理员' || (form['delivery_users_id'] && $check_field('set','user_gender')) || (!form['delivery_users_id'] && $check_field('add','user_gender'))"
                ></uni-data-select>
                <!-- 仅查看 -->
...
export default {
  ...
  data() {
    return {
      ...
      group_table: "delivery_users",
      is_password: true,
      field: "delivery_users_id",
      url_add: "~/api/delivery_users/add?",
      url_set: "~/api/delivery_users/set?",
      url_get_obj: "~/api/delivery_users/get_obj?",
      url_upload: "~/api/delivery_users/upload?",
  ...
  methods: {
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/delivery_users/upload?'),
    ...
    submit_sub(form_sub){
      if(form_sub["delivery_users_id"]){
        // 提交事件
        this.$post("~/api/" + this.group_table + "/set?delivery_users_id=" + form_sub["delivery_users_id"],form_sub,(res)=>{
    ...
    is_view(){
      ...
      if(!bl){
        bl = this.$check_action('/delivery_users/table','add');
        console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
      }
      if(!bl){
        bl = this.$check_action('/delivery_users/table','set');
        console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
      }
      if(!bl){
        bl = this.$check_action('/delivery_users/view','add');
        console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
      }
      if(!bl){
        bl = this.$check_action('/delivery_users/view','set');
        console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
      }
      if(!bl){
        bl = this.$check_action('/delivery_users/view','get');
