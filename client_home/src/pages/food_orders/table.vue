				...
				<view v-for="(o,i) in list" class="row">
                            ...
                            <span>购买数量</span>
								...
								<text>
									{{ o['purchase_quantity'] }}
													...
													<view v-if="0 && $check_field('get','total_purchase_price')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>购买总价</span>
                          </view>
                            ...
                            <span>订单备注</span>
								...
								<text>
									{{ o["order_remarks"] }}
													...
													<view v-if="1 && $check_field('get','order_status')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>订单状态</span>
                          </view>
                  ...
                  <view class="col">
					...
					<navigator  :url="'/pages/food_orders/view?' + field + '=' + o[field]"
	                    ...
	                    <a @click="goto_pay(o)" size="mini"  v-if="o.pay_state==='未支付' && $check_pay('/food_orders/table')" class='btn btn-primary'>支付</a>
	                                    <!--跨表按钮-->
                                <a type="button" size="mini" class="btn btn-primary" v-if="$check_action('/delivery_order/view','add')" @click="to_form('/pages/delivery_order/view',o)">
                      <span>下配送单</span>
                    </a>
                        					</view>
				</view>
...
<script>
    ...
    return param;
        ...
        async get_list_user_student_users() {
          ...
          goto_pay(v) {
            //打开选择支付方式
            this.pay_obj.id = v.food_orders_id
            this.model=true;
          },
          choosepayway(idx){
            switch(idx){
          ...
          submit_pay(){
            let _this = this;
            let url = this.$toUrl(this.query, "~/api/food_orders/set?food_orders_id="+this.pay_obj.id);
            let param = {"pay_state":"已支付","pay_type":this.pay_obj.payActiveName}
            this.$post(url,param,(res)=>{
              if(res.result){
                _this.$toast("支付成功");
                setTimeout(()=>{
                  _this.$nav("/pages/food_orders/table");
                }, 700);
              }else if(res.error){
