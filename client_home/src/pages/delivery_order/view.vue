          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','order_number') || ($check_field('add','order_number') || $check_field('set','order_number'))" label="订单编号" name="order_number">
                            <uni-easyinput type="text" v-model="form['order_number']" v-if="user_group === '管理员' || (form['delivery_order_id'] && $check_field('set','order_number')) || (!form['delivery_order_id'] && $check_field('add','order_number'))" :disabled="disabledObj['order_number_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','store_name') || ($check_field('add','store_name') || $check_field('set','store_name'))" label="店铺名称" name="store_name">
                            <uni-easyinput type="text" v-model="form['store_name']" v-if="user_group === '管理员' || (form['delivery_order_id'] && $check_field('set','store_name')) || (!form['delivery_order_id'] && $check_field('add','store_name'))" :disabled="disabledObj['store_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','merchant_users') || ($check_field('add','merchant_users') || $check_field('set','merchant_users'))" label="商家用户" name="merchant_users">
                  ...
                  v-if="user_group === '管理员' || (form['delivery_order_id'] && $check_field('set','merchant_users')) || (!form['delivery_order_id'] && $check_field('add','merchant_users'))"
                ></uni-data-select>
                <uni-data-select
                  v-model="form['merchant_users']"
                  :localdata="list_user_merchant_users"
              ...
              <uni-forms-item v-if="$check_field('get','food_name') || ($check_field('add','food_name') || $check_field('set','food_name'))" label="美食名称" name="food_name">
                            <uni-easyinput type="text" v-model="form['food_name']" v-if="user_group === '管理员' || (form['delivery_order_id'] && $check_field('set','food_name')) || (!form['delivery_order_id'] && $check_field('add','food_name'))" :disabled="disabledObj['food_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','food_types') || ($check_field('add','food_types') || $check_field('set','food_types'))" label="美食类型" name="food_types">
                            <uni-easyinput type="text" v-model="form['food_types']" v-if="user_group === '管理员' || (form['delivery_order_id'] && $check_field('set','food_types')) || (!form['delivery_order_id'] && $check_field('add','food_types'))" :disabled="disabledObj['food_types_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','food_prices') || ($check_field('add','food_prices') || $check_field('set','food_prices'))" label="美食价格" name="food_prices">
                            <uni-easyinput type="text" v-model="form['food_prices']" v-if="user_group === '管理员' || (form['delivery_order_id'] && $check_field('set','food_prices')) || (!form['delivery_order_id'] && $check_field('add','food_prices'))" :disabled="disabledObj['food_prices_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','student_users') || ($check_field('add','student_users') || $check_field('set','student_users'))" label="学生用户" name="student_users">
                  ...
                  v-if="user_group === '管理员' || (form['delivery_order_id'] && $check_field('set','student_users')) || (!form['delivery_order_id'] && $check_field('add','student_users'))"
                ></uni-data-select>
                <uni-data-select
                  v-model="form['student_users']"
                  :localdata="list_user_student_users"
              ...
              <uni-forms-item v-if="$check_field('get','student_name') || ($check_field('add','student_name') || $check_field('set','student_name'))" label="学生姓名" name="student_name">
                            <uni-easyinput type="text" v-model="form['student_name']" v-if="user_group === '管理员' || (form['delivery_order_id'] && $check_field('set','student_name')) || (!form['delivery_order_id'] && $check_field('add','student_name'))" :disabled="disabledObj['student_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','contact_number') || ($check_field('add','contact_number') || $check_field('set','contact_number'))" label="联系电话" name="contact_number">
                            <uni-easyinput type="text" v-model="form['contact_number']" v-if="user_group === '管理员' || (form['delivery_order_id'] && $check_field('set','contact_number')) || (!form['delivery_order_id'] && $check_field('add','contact_number'))" :disabled="disabledObj['contact_number_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','shipping_address') || ($check_field('add','shipping_address') || $check_field('set','shipping_address'))" label="收货地址" name="shipping_address">
                            <uni-easyinput type="text" v-model="form['shipping_address']" v-if="user_group === '管理员' || (form['delivery_order_id'] && $check_field('set','shipping_address')) || (!form['delivery_order_id'] && $check_field('add','shipping_address'))" :disabled="disabledObj['shipping_address_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','purchase_quantity') || ($check_field('add','purchase_quantity') || $check_field('set','purchase_quantity'))" label="购买数量" name="purchase_quantity">
                            <uni-easyinput type="text" v-model="form['purchase_quantity']" v-if="user_group === '管理员' || (form['delivery_order_id'] && $check_field('set','purchase_quantity')) || (!form['delivery_order_id'] && $check_field('add','purchase_quantity'))" :disabled="disabledObj['purchase_quantity_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','delivery_users') || ($check_field('add','delivery_users') || $check_field('set','delivery_users'))" label="配送用户" name="delivery_users">
                  ...
                  v-if="user_group === '管理员' || (form['delivery_order_id'] && $check_field('set','delivery_users')) || (!form['delivery_order_id'] && $check_field('add','delivery_users'))"
                ></uni-data-select>
                <uni-data-select
                  v-model="form['delivery_users']"
                  :localdata="list_user_delivery_users"
              ...
              <uni-forms-item v-if="$check_field('get','delivery_status') || ($check_field('add','delivery_status') || $check_field('set','delivery_status'))" label="配送状态" name="delivery_status">
                    <uni-data-select
                  v-model="form.delivery_status"
                  :localdata="list_delivery_status"
                  :clear="!disabledObj['delivery_status_isDisabled']"
                  :disabled="disabledObj['delivery_status_isDisabled']"
                ...
                ></uni-data-select>
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','delivery_status')">
                  {{ form['delivery_status'] }}
                </text>
                  </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','delivery_description') || ($check_field('add','delivery_description') || $check_field('set','delivery_description'))" label="配送描述" name="delivery_description">
                    <uni-easyinput type="textarea" v-model="form['delivery_description']" v-if="user_group === '管理员' || (form['delivery_order_id'] && $check_field('set','delivery_description')) || (!form['delivery_order_id'] && $check_field('add','delivery_description'))" :disabled="disabledObj['delivery_description_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','delivery_description')">
                  {{ form['delivery_description'] }}
                </text>
                  </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','receiving_status') || ($check_field('add','receiving_status') || $check_field('set','receiving_status'))" label="收货状态" name="receiving_status">
                    <uni-data-select
                  v-model="form.receiving_status"
                  :localdata="list_receiving_status"
                  :clear="!disabledObj['receiving_status_isDisabled']"
                  :disabled="disabledObj['receiving_status_isDisabled']"
                ...
                ></uni-data-select>
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','receiving_status')">
                  {{ form['receiving_status'] }}
                </text>
                  </uni-forms-item>
...
export default {
  ...
  methods: {
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/delivery_order/upload?'),
    ...
    async get_list_delivery_status() {
          ['待取餐','取餐中','配送中','已完成'].map((o) => this.list_delivery_status.push({value:o,text:o}));
            },
     ...
     * 获取收货状态列表
     */
    async get_list_receiving_status() {
          ['确认收货'].map((o) => this.list_receiving_status.push({value:o,text:o}));
            },
  ...
  created() {
                    ...
                    this.get_list_delivery_status();
                        this.get_list_receiving_status();
          },
}
</script>
